...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (addr.GetPort() != 33472) return;
        } else if (addr.GetPort() == 33472)
            return;
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 Delectrum tx got MASTERNODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000000 DLTR tx -> 1 confirmation
            ...
            //REJECT MASTERNODE ENTRY IF EXITSTING TOTAL MASTERNODES ARE 2000
            if(vMasternodes.size() >= 2000 ) {
                LogPrint("masternode", "Max masternode limit reached!");
                return;
            } else {
